...
/** 路径段数组 → registry key */
export function pathToRegistryKey(path: string[], extension?: "md" | "mdx"): string {
  return `articles/${path.join("/")}.${extension ?? "mdx"}`;
}
...
/** 路径段数组 → registry entry 与文件扩展名（.mdx 优先，其次 .md）；都不存在返回 null */
export function findArticle(
  path: string[],
): { entry: ArticleRegistryEntry; extension: "md" | "mdx" } | null {
  for (const extension of ["mdx", "md"] as const) {
    const entry = registry[pathToRegistryKey(path, extension)] as ArticleRegistryEntry | undefined;
