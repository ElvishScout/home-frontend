...
2017 年的论文 *Attention Is All You Need* 做了一件激进的事：把 RNN 整个扔掉，只留注意力与前馈网络。这个决定同时兑现了上一篇的两条红利——任意位置一步直达、训练全并行；剩下的两条代价（没有顺序感、$O(n^2)$），一个用位置编码补上，一个留待后人优化。这篇把整张架构图走一遍。它是本系列的终点，也是下一个系列的起点。
